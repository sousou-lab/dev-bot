...
└──────────────────────┬──────────────────────────────┘
   ...
   (Read/Grep/Glob のみ)         (workspace 内書き込み)
         ...
         └─────────────┬─────────────┘
                       │
              pipeline.py  ← workspace準備 → 実行 → 検証 → レビュー → PR
                       │
         ┌─────────────┼─────────────┐
         │             │             │
...
Issue ごとに独立した Git worktree を作成し、ブランチ `agent/gh-{issue_number}-{slug}` で作業する。bare mirror リポジトリを共有することで clone コストを最小化。retry 時も同じ worktree を再利用する。
